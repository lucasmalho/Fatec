...
export function Schedule() {
  ...
  useEffect(() => {
    ...
    // Verifique se o usuário existe na tabela de clientes
    const checkUsExists = async () => {
      ...
      if (error || !data) {
        navigate('/login', { 
          state: { 
            returnTo: '/agendar',
            error: 'Você precisa estar cadastrado como cliente para agendar exames.'
          }
        });
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedLab || !selectedDate || !selectedTime || !user) {
    ...
    try {
      ...
      if (userError || !userData) {
        throw new Error('Usuário não encontrado como cliente. Por favor, faça login novamente.');
      }
      ...
      // Redirecionar página confirmação agendamento
      navigate('/exame-agendado', {
        state: {
          appointmentDetails: {
            laboratory: selectedLab.nome,
            date: appointmentDateTime.toISOString(),
            time: selectedTime,
            examType: examType,
            address: `${selectedLab.endereco}, ${selectedLab.bairro}, ${selectedLab.cidade} - ${selectedLab.uf}`
          }
        }
      });
    } catch (err) {
      console.error('Erro ao salvar agendamento:', err);
      setError(err instanceof Error ? err.message : 'Erro ao realizar agendamento. Por favor, tente novamente.');
    } finally {
